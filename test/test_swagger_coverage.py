...
class TestSwaggerCoverage(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.BANNED_RULES = ['/specs.json', '/apidocs', '/static']
        cls.THRESHOLD = 100
    ...
    def test_swagger_coverage(self):
        ...
        rules = \
            self.filter_rules(self.client.application.url_map.iter_rules())

        (covered_methods, methods_total) = \
        ...
        self.color_print(end_color, '%s of %s routes are swagged'
                         % (str(covered_methods), str(methods_total)))
    ...
    def retrieve_swagger_specs(self):
        ...
        with warnings.catch_warnings():
            ...
            response = self.client.get('swagger/specs.json')
        ...
        for rule in rules:
            ...
            for method in methods:
                ...
                try:
                    ...
                    self.color_print('red', 'Uncovered: %s of route %s'
                                     % (method, parsed_rule))
                    continue
    ...
    def filter_methods(methods):
        ...
        return [
            method for method in methods if method not in [
                'OPTIONS', 'HEAD']]
    ...
    def is_banned_rule(self, rule):
        ...
        return any(banned_rule in str(rule) for banned_rule in
                   self.BANNED_RULES)
